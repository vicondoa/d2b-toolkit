...
impl RawColor {
    fn into_color(self) -> Result<RgbaColor, ()> {
        match self {
            Self::Hex(value) => RgbaColor::from_str(&value).map_err(|_| ()),
...
mod tests {
    use super::{
        css_variables, parse_ui_colors_json, parse_ui_colors_json_with_fallbacks, ColorFallback,
        ColorRole, FallbackReason, UiColorsArtifact,
    };
    use d2b_wayland_core::RgbaColor;

    #[test]
    fn parses_nested_palette_with_hex_and_component_colors() {
    ...
    #[test]
    fn applies_defaults_for_missing_roles() {
        let parsed =
            parse_ui_colors_json_with_fallbacks(r##"{ "palette": { "host": "#000000" } }"##)
                .unwrap();

        assert_eq!(parsed.artifact.palette.host, RgbaColor::rgb(0, 0, 0));
        assert!(parsed.fallbacks.contains(&ColorFallback {
            role: ColorRole::Vm,
            reason: FallbackReason::Missing
        }));
    }
    ...
    #[test]
    fn preserves_legacy_artifact_entry_point() {
        let artifact = parse_ui_colors_json(r##"{ "host": "#010203" }"##).unwrap();
        assert_eq!(artifact.palette.host, RgbaColor::rgb(1, 2, 3));
    }
